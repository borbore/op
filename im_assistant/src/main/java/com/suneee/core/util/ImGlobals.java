...
public class ImGlobals {
	...
	private static final Logger Log = LoggerFactory.getLogger(ImGlobals.class);
    ...
    private static String subscriptionMode = null;
    
    private static Map remoteServiceParams = null;
    private static Map remoteServiceParams_c = null;

    private static String remoteServiceClassName = null;
    private static String remoteServiceClassName_c = null;
    /**
    ...
    public static Map getXMLPropertiesMap(String parent) {
        if (xmlProperties == null) {
            loadSetupProperties();
        }
        if (xmlProperties == null) {
    ...
    public static List<Map<String, String>> getRobotXMLProperties(String parentsHeard){
    	List<Map<String, String>> list = new ArrayList<Map<String, String>>();
         ...
         return xmlProperties.getRobotProperties(parentsHeard);
    }
    ...
    private synchronized static void loadSetupProperties() {
        if (xmlProperties == null) {
                try {
                    xmlProperties = new XMLProperties(ImConstants.REPLYROBOT_CONFIG_FILENAME);
                }
     ...
     * @param messageBody
     */
    public static void sendMessageToAdmin(Connection conn,String messageSubject, String messageBody){
    	String adminAccount = getXMLProperty(ImConstants.ADMINACCOUNT);
		String subject = messageSubject == null ? "Message From Robot" : messageSubject;
		sendMessage(conn, subject, messageBody, adminAccount);
    }
     ...
     * @param targetJID
     */
	public static void sendMessage(Connection conn, String messageSubject,
			String messageBody, String targetJID) {
		Message message = new Message(targetJID);
     ...
     * @param targetJID
     */
	public static void sendMessage(Connection conn,String targetJID) {
		Message message = new Message(targetJID);
		conn.sendPacket(message);
	}
	
	public static void sendMessage(Connection conn, String body, String targetJID) {
		Message message = new Message(targetJID);
     ...
     * @param status
     */
    public static void sendPresenceStatus(Connection conn, String status){
    ...
    /**
     * 根据key从licence中取得value
     * @param key
     * @return
     */
    public static String getValueByKeyFromLicence(String key){
    	if(key.equals(ImConstants.ROBOTJID_COUNT_MAX)){
    		return "10";
    	}else if(key.equals(ImConstants.VCODE)){
    		return "123456";
    	}else if(key.equals(ImConstants.XMPP_RESOURCE)){
    		return "robot";
    	}else if(key.equals(ImConstants.FRIEND_COUNT_MAX)){
    		return "500";
    	}
    	return null;
    }
     ...
     * @return
     */
    public static final String getSubscriptionMode(){
    	if(subscriptionMode == null){
    		subscriptionMode = getXMLProperty("contact.subscriptionMode");
    	}
     ...
     * @return
     */
    public static final Map getRemoteServiceParamsMap(){
    	if(remoteServiceParams == null){
    		remoteServiceParams = getXMLPropertiesMap("remoteService.params");
    	}
    	return remoteServiceParams;
    }
    
    public static final Map getRemoteServiceParamsMap_c(){
    	if(remoteServiceParams_c == null){
    		remoteServiceParams_c = getXMLPropertiesMap("remoteService.params_c");
    	}
    	return remoteServiceParams_c;
    }
     ...
     * @return
     */
    public static final String getRemoteClassName(){
    	if(remoteServiceClassName == null){
    		remoteServiceClassName = getXMLProperty("remoteService.classname");
    	}
    	return remoteServiceClassName;
    }
    
    public static final String getRemoteClassName_c(){
    	if(remoteServiceClassName_c == null){
    		remoteServiceClassName_c = getXMLProperty("remoteService.classname_c");
    	}
    	return remoteServiceClassName_c;
    }
     ...
     */
    public static void setXMLPropertiesList(String name, List<Map<String, String>> propertiesList) {
        if (xmlProperties == null) {
            loadSetupProperties();
        }

        if (xmlProperties != null) {
        		xmlProperties.setPropertiesList(name, propertiesList);
        }
    }
}